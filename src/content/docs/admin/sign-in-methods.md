...
Every single sign-on connection you add carries a **name**, and that name is the button members press. Use the name your organisation already uses for the provider ("Acme SSO", "Corp IdP") so members recognise it on the sign-in screen. The field shows the words it will become: leave it empty and it reads "Sign in with single sign-on", which is what the button would say.
...
- **Stance** — second factors *required* or *optional* for password sign-ins. **Require two-factor authentication** shows who it would lock out (members with no enrolled factor, counted from the roster) before you commit; turning it off asks for confirmation too.
...
- Methods the policy excludes are refused at sign-in and at enrolment, not merely hidden.
