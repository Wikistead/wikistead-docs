...
**既定では PlantUML のブロックはソースのまま表示されます。**配備に描画サービスが設定されていれば、同じブロックが図として描かれます。運用者に確認するか、[環境変数リファレンス](/ja/reference/environment-variables/)の `PLANTUML_RENDER_URL` を参照してください。どちらの場合もページに書かれているテキストは同じなので、ドキュメントを書き換えずに別の環境へ持っていけます。
